...
bench_buildIP_list =
  [ bench "buildIP-list-foldr" $ nf (map buildIP_foldr) ipcomps,
    bench "buildIP-list-foldl" $ nf (map buildIP_foldl) ipcomps,
    bench "buildIP-list-foldl-shl" $ nf (map buildIP_foldl_shl) ipcomps
  ]
  where
    ipcomps =
      [ [0, 0, 0, 1],
