...
instance Show (Floor mx cur) where
  show MkFloor = "Floor " <> show (snatToNat (snat :: SNat cur)) <> " of " <> show (snatToNat (snat :: SNat mx))
...
sameFloor :: forall mx to from. Floor mx to -> Floor mx from -> Maybe (to :~: from)
