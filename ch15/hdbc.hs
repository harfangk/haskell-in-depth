...
catIdByName conn catName =
  quickQuery conn "SELECT category_id FROM category WHERE name = ?" [toSql catName]
...
isAssigned conn cid fid = do
  res <- quickQuery conn ("SELECT count(category_id) FROM film_category WHERE category_id = ? AND film_id= ?") [toSql cid, toSql fid]
...
main = withPostgreSQL connString (handleSqlError . demo)
 where
   connString = "user=postgres password=postgres host=localhost dbname=sakila_films"
